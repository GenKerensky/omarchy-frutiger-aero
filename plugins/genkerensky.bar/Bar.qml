...
Item {
  ...
  //
  // NOT `required`, deliberately. shell.qml loads a custom bar through a
  // Loader `source:` URL, and a Loader cannot create a component whose root
  // declares required properties -- those must be bound at construction,
  // while the host injects them from configureBar() in onLoaded, one step
  // too late. Leaving them required makes the bar fail with "Required
  // property X was not initialized" and draw nothing. Defaults below are
  // only ever seen for the instant before the host injects the real values.
  ...
  component BarPanel: PanelWindow {
    ...
    // Frutiger Aero gloss.
    //
    // The strip's own fill is a single flat colour: Color.bar.background
    // resolves to one QML colour, and a gradient handed to it in shell.toml
    // would collapse to its first stop. Gradients in tokens are a border
    // feature; fills have to be painted. So the gloss is painted here, on
    // top of that fill.
    //
    // Every value below is a tint, a shade, or the theme accent -- nothing
    // is hardcoded to aquamarine. The bar glosses in whatever colour the
    // current theme sets, so it survives a theme switch without a hook.
    //
    // Declared before the Loader, so widgets draw over it. No input items
    // here, so it never steals a click or a hover from the bar.
    // ------------------------------------------------------------------
    Item {
      id: gloss
      ...
      // Thickness of the edge lines, in the same scaled units the rest of
      // the bar uses, so gloss tracks the font/spacing scale.
      readonly property int edge: Math.max(1, Math.round(Style.spaceReal(0.5)))

      // Which side faces the desktop rather than the screen edge.
      readonly property bool innerAtStart: root.position === "bottom" || root.position === "right"

      // Moulded fill: lit above the midline, shaded below it. The hard break
      // at 0.5 is the whole trick -- a smooth fade reads as a flat panel, a
      // discontinuity reads as a curved surface catching a light above it.
      Rectangle {
        anchors.fill: parent
        gradient: Gradient {
          orientation: root.vertical ? Gradient.Horizontal : Gradient.Vertical
          GradientStop { position: 0.00; color: Qt.rgba(1, 1, 1, 0.17) }
          GradientStop { position: 0.49; color: Qt.rgba(1, 1, 1, 0.05) }
          GradientStop { position: 0.51; color: Qt.rgba(0, 0, 0, 0.07) }
          GradientStop { position: 1.00; color: Qt.rgba(0, 0, 0, 0.14) }
        }
      }

      // Specular cap over the lit half.
      Rectangle {
        width: root.vertical ? Math.round(parent.width * 0.42) : parent.width
        height: root.vertical ? parent.height : Math.round(parent.height * 0.42)
        gradient: Gradient {
          orientation: root.vertical ? Gradient.Horizontal : Gradient.Vertical
      ...
      // Bright hairline along the screen edge -- the top of the moulding.
      Rectangle {
        width: root.vertical ? gloss.edge : parent.width
        height: root.vertical ? parent.height : gloss.edge
        x: root.vertical && !gloss.innerAtStart ? 0 : (root.vertical ? parent.width - gloss.edge : 0)
        y: root.vertical ? 0 : (gloss.innerAtStart ? parent.height - gloss.edge : 0)
        color: Qt.rgba(1, 1, 1, 0.28)
      }

      // Lit edge facing the desktop, tinted with the theme accent. This is
      // the line that makes the strip read as an object with a light on it
      // rather than as a band of colour.
      Rectangle {
        width: root.vertical ? gloss.edge : parent.width
        height: root.vertical ? parent.height : gloss.edge
        x: root.vertical ? (gloss.innerAtStart ? 0 : parent.width - gloss.edge) : 0
        y: root.vertical ? 0 : (gloss.innerAtStart ? 0 : parent.height - gloss.edge)
    ...
    Component {
      ...
      Item {
        ...
        LeftModules {
          ...
          // Zero, not Style.space(8): the Aero menu button is drawn flush into
          // the top-left corner, square on the screen edge, the way a start
          // button always was. It supplies the inset itself. Restore
          // Style.space(8) here if that button ever leaves the left section,
          // or whatever sits first will sit hard against the screen edge.
